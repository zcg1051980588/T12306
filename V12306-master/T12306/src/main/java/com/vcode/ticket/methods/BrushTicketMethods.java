...
public class BrushTicketMethods extends Thread{
	...
	private Logger Log = Logger.getLogger(BrushTicketMethods.class.getName());
	...
	public void run() {
		...
		while (home_page.result) {
			...
			brushVotes();
			try {
				Log.info("休眠时间："+home_page.brushVotesFrequency.getValue().toString());
